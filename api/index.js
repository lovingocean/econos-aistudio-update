...
var SovereignBillingProvider = class {
  ...
  async createCustomer(orgId, email, name) {
    ...
    const customerId = `cus_${crypto2.createHash("sha256").update(`${orgId}:${email}`).digest("hex").slice(0, 16)}`;
    return { customerId };
  }
  async createCheckoutSession(params) {
    const sessionId = `cs_${crypto2.randomBytes(16).toString("hex")}`;
  ...
  verifyWebhookSignature(rawBody, signature, secret) {
    ...
    try {
      if (signature.includes("t=") && signature.includes("v1=")) {
        ...
        const expectedHash = crypto2.createHmac("sha256", sec).update(signedPayload).digest("hex");
        return crypto2.timingSafeEqual(Buffer.from(signatureHash), Buffer.from(expectedHash));
      } else {
        const expectedHash = crypto2.createHmac("sha256", sec).update(rawBody).digest("hex");
        if (signature.length !== expectedHash.length) return false;
        return crypto2.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedHash));
      }
    } catch {
      return false;
    }
  ...
  generateTestSignature(payload, secret) {
    ...
    const hash = crypto2.createHmac("sha256", sec).update(signedPayload).digest("hex");
...
apiRouter.use((req, res, next) => {
  ...
  const isPublicRoute = req.path.startsWith("/auth") || req.path.startsWith("/health") || req.path.startsWith("/billing/webhook") || req.path === "/organizations";
