...
export class EconosAIAdvisorService {
  ...
  public async consultAdvisor(context: WealthAdvisorContext): Promise<StructuredAdvisorResponse> {
    // If Gemini API is available, invoke gemini-2.5-flash with structured system instructions
    if (this.genAIClient) {
      try {
        return await this.callGeminiModel(context);
      } catch (err: any) {
        console.info(`[ECONOS AI] Primary Gemini call unavailable (${err?.status || err?.message || 'high demand'}). Using sovereign deterministic economic reasoning engine.`);
...
`;
    ...
    let response;
    try {
      response = await this.genAIClient.models.generateContent({
      ...
      // Fallback model trial before deterministic engine
      response = await this.genAIClient.models.generateContent({
