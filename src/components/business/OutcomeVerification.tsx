                  ...
                  <div className="lg:w-72 bg-zinc-950 p-4 rounded-lg border border-zinc-800 flex flex-col justify-between">
                    <div className="space-y-2">
                      <div className="flex justify-between text-zinc-400 text-[11px]">
                        ...
                        <span className="text-zinc-200">${(v.expectedFinancialImpact ?? 0).toLocaleString()}</span>
                      </div>

                      <div className="flex justify-between text-zinc-400 text-[11px]">
                        <span>Actual Audited Impact:</span>
                        <span className="text-zinc-100 font-bold">${(v.actualFinancialImpact ?? 0).toLocaleString()}</span>
                      </div>
                      ...
                      <div className="pt-2 border-t border-zinc-850">
                        ...
                        <div className={`text-xl font-bold flex items-center gap-1 ${
                          ...
                          <span>
                            {isPositiveVariance ? `+$${(v.variance ?? 0).toLocaleString()}` : `-$${Math.abs(v.variance ?? 0).toLocaleString()}`}
                          </span>
                        </div>
            ...
            <form onSubmit={handleCreateVerification} className="space-y-3 text-xs">
              <div>
                ...
                >
                  ...
                  {opportunities.map(o => (
                    <option key={o.id} value={o.id}>{o.title} (${(o.estimatedImpact ?? 0).toLocaleString()})</option>
                  ))}
                </select>
              </div>
