          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded font-mono text-[11px] font-bold bg-amber-50 text-amber-800 border border-amber-200">
                {results ? `${results.totalTests}-POINT AUDIT SUITE` : '32-POINT AUDIT SUITE'}
              </span>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Authoritative mathematical tests validating signups, trials, upgrades, webhook HMAC, multi-bank reconciliation, balanced GL, dual-control RBAC, AI CFO anomalies, FX forwards, cap table waterfall, board deck metrics, P2P 3-way matching, ASC 606 revenue recognition, continuous close lock, and SOC-2 audit vault.
            </p>
          </div>
            ...
            >
              {isRunning ? (
                <>
                  ...
                  <span>Executing {results?.totalTests || 24}-Point Verification...</span>
                </>
              ) : (
                <>
                  <Play className="w-4 h-4 fill-white" />
                  <span>{results ? `Re-run ${results.totalTests}-Point Verification` : 'Execute All 24 Commercial Tests'}</span>
                </>
              )}
            </button>
          ...
          <div className={`px-6 py-3 border-b flex items-center justify-between text-xs font-mono ${
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold">
                {results.allPassed ? `ALL ${results.totalTests} COMMERCIAL CONSTRAINTS PASSED` : `${results.failedCount} TESTS FAILED`}
              </span>
            </div>
            <div className="flex items-center gap-4">
              <span>Passed: <strong className="text-emerald-700">{results.passedCount}</strong>/{results.totalTests}</span>
          ...
          {!results && !isRunning && (
            <div className="p-16 text-center text-slate-500 space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Press the button above to execute automated validation across all 24 pricing, subscription, webhook HMAC, multi-tenant isolation, multi-bank reconciliation, balanced double-entry ledger, dual-control RBAC, and what-if Monte Carlo paths.
              </p>
            </div>
          )}
          ...
          {isRunning && (
            <div className="p-16 flex flex-col items-center justify-center text-slate-500 space-y-3">
              ...
              <div className="font-mono text-xs text-slate-800 font-bold">
                Executing {results?.totalTests || 24} Automated Commercial Proofs...
              </div>
              <div className="text-[11px] text-slate-500 font-mono">
                Checking signatures, plan boundaries, quota enforcement, reconciliation, GL integrity, and stress tolerances
              </div>
            </div>
          )}
