                ...
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className="text-slate-400">{inc.createdAt ? new Date(inc.createdAt).toLocaleDateString() : (inc.detectedAt ? new Date(inc.detectedAt).toLocaleDateString() : 'Recent')}</span>
                    </div>

                    <h3 className="text-base font-bold text-slate-900 font-sans">{inc.title || inc.category || 'Security Incident'}</h3>
                    <p className="text-slate-600 text-xs mt-1 font-sans leading-relaxed">{inc.description}</p>

                    <div className="mt-2 text-[11px] text-slate-500">
                      Policy Tripped: <span className="text-amber-800 font-bold">{inc.policyTripped || inc.actionAttempted || 'Standard Security Guardrail'}</span>
                    </div>
