...
type TrustSubTab = 'REGISTRY' | 'FIREWALL' | 'APPROVALS' | 'INCIDENTS' | 'AUDIT' | 'HARDWARE_SECURITY';

export const TrustWorkspace: React.FC = () => {
  const { currentOrg } = useAuth();
  const [subTab, setSubTab] = useState<TrustSubTab>('REGISTRY');
  const [showDirectHardwareModal, setShowDirectHardwareModal] = useState(false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200/90 shadow-2xs overflow-x-auto no-scrollbar">
          ...
          <button
            id="trust-subtab-hardware"
            onClick={() => setSubTab('HARDWARE_SECURITY')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-medium transition ${
              subTab === 'HARDWARE_SECURITY' 
                ? 'bg-slate-100 text-slate-900 font-bold shadow-2xs' 
                : 'text-slate-500 hover:text-slate-800'
            }`}
          >
            <Key className="w-3.5 h-3.5 text-sky-600" />
            <span>6. Hardware Security (FIDO2)</span>
          </button>
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono uppercase tracking-widest text-slate-500 font-bold">FIDO2 / WebAuthn Physical Vault</span>
                </div>
                <h2 className="text-lg font-bold text-slate-900 mt-1">Hardware-Anchored Dual Custody</h2>
                <p className="text-xs text-slate-500 mt-0.5 max-w-2xl">
                  Enforces WebAuthn/FIDO2 biometric physical presence checks (YubiKey 5 FIPS / Titan Security Key) for all Class C/D autonomous delegations over $50,000.
                </p>
              </div>

              <button
                onClick={() => setShowDirectHardwareModal(true)}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#132338] hover:bg-[#0c1827] text-white font-mono text-xs font-bold transition shadow-xs"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-xs">
            <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-2">
              <div className="flex items-center gap-2 text-emerald-700 font-bold">
                ...
                <span>Level 3 HSM Attestation</span>
              </div>
              <p className="text-slate-500 text-[11px]">
                Private key material never leaves the secure enclave (SE). All ECDSA / Ed25519 signatures are generated inside hardware.
              </p>
              <div className="pt-2 text-[10px] text-slate-400">
                Specification: FIPS 140-3 Level 3
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-2">
              <div className="flex items-center gap-2 text-sky-700 font-bold">
                ...
                <span>SOX 404 Merkle Proof</span>
              </div>
              <p className="text-slate-500 text-[11px]">
                Every physical touch creates an immutable leaf in the ECONOS SHA-256 Merkle tree, cryptographically binding user identity to the agent action.
              </p>
              <div className="pt-2 text-[10px] text-slate-400">
                Proof Format: RFC 6962 CT log format
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-2">
              <div className="flex items-center gap-2 text-amber-700 font-bold">
                ...
                <span>Dual-Custody Threshold</span>
              </div>
              <p className="text-slate-500 text-[11px]">
                Operations above $250,000 require 2-of-3 quorum across CFO, Head of Treasury, and Risk Officer hardware keys.
              </p>
              <div className="pt-2 text-[10px] text-slate-400">
                Enforcement: Kernel-level gating
              </div>
            </div>
          </div>
        </div>
      ...
      {showDirectHardwareModal && (
        <HardwareSecurityModal
          agentId="agt_sys_root"
          agentName="Root Treasury Fiduciary Key"
          actionTitle="Simulated Hardware Assertion & Dual-Custody"
          amountUsd={250000}
          onClose={() => setShowDirectHardwareModal(false)}
          onSuccess={(sig) => {
            alert(`Hardware assertion confirmed!\nEd25519 signature:\n${sig}`);
            setShowDirectHardwareModal(false);
          }}
        />
      )}
    </div>
