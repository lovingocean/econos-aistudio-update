...
export const WealthDashboard: React.FC<WealthDashboardProps> = ({ profile, onRefresh, onOpenAdvisor }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSaveError(err.message || 'Failed to update wealth profile');
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap items-baseline gap-5">
              <div>
                ...
                <span className="text-3xl font-black font-mono text-slate-900 tracking-tight">${(totalNetWorth || 0).toLocaleString()}</span>
              </div>
              ...
              <div>
                ...
                <span className="text-2xl font-black font-mono text-amber-600 tracking-tight">${(wealthGap || 0).toLocaleString()}</span>
              </div>
              ...
              <div>
                ...
                <span className="text-xl font-bold font-mono text-slate-700">
                  ${(target || 0).toLocaleString()} <span className="text-xs text-slate-400 font-normal">by age {profile?.targetRetirementAge || 52}</span>
                </span>
              </div>
            </div>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs">
          ...
          <div className="space-y-3 font-mono text-xs">
            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              ...
              <span className="font-bold text-emerald-700">${(liquid || 0).toLocaleString()}</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-slate-600">Operating Business Equity</span>
              <span className="font-bold text-slate-900">${(equity || 0).toLocaleString()}</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-slate-600">Illiquid Assets (Real Estate, IP)</span>
              <span className="font-bold text-slate-700">${(illiquid || 0).toLocaleString()}</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-slate-600">Total Liabilities & Debt</span>
              <span className="font-bold text-rose-600">-${(debt || 0).toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs">
          ...
          <div className="space-y-3 font-mono text-xs">
            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              ...
              <span className="font-bold text-slate-800">${(activeIncome || 0).toLocaleString()}/mo</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-slate-600">Passive Capital Yields</span>
              <span className="font-bold text-emerald-700">+${(passiveIncome || 0).toLocaleString()}/mo</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-200">
              <span className="text-slate-600">Household Living Expenses</span>
              <span className="font-bold text-rose-600">-${(monthlyExpenses || 0).toLocaleString()}/mo</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 rounded-xl bg-[#f0f7f3] border border-[#d7e9dc] text-emerald-800 font-bold">
              ...
              <span>+${(netMonthlySurplus || 0).toLocaleString()}/mo</span>
            </div>
          </div>
        </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-lg w-full shadow-2xl">
            ...
            {saveError && (
              <div className="mb-3 p-2.5 rounded-lg bg-rose-50 border border-rose-200 text-rose-700 text-xs font-mono">
                {saveError}
              </div>
            )}
